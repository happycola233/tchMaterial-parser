...
def download_file(url: str, save_path: str) -> None: # 下载文件
    ...
    if all(state["finished"] for state in download_states):
        ...
        if len(failed_urls) > 0:
            # 方案1: 使用 join() 前先将字符串存储在变量中
            failed_str = '\n'.join(failed_urls)
            messagebox.showwarning("下载完成", f"文件已下载到：{os.path.dirname(save_path)}\n以下链接下载失败：\n{failed_str}")

            # 方案2: 不使用 f-string,改用 format() 方法
            # messagebox.showwarning("下载完成", "文件已下载到：{}\n以下链接下载失败：\n{}".format(
            #     os.path.dirname(save_path), 
            #     '\n'.join(failed_urls)
            # ))
        else:
            messagebox.showinfo("下载完成", f"文件已下载到：{os.path.dirname(save_path)}") # 显示完成对��框
