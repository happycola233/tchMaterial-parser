...
def parse_and_copy() -> None: # 解析并复制链接
    urls = [line.strip() for line in url_text.get("1.0", "end").splitlines() if line.strip()] # 获取所有非空行
    resource_links = []
    failed_links = []
...
def download() -> None: # 下载资源文件
    ...
    urls = [{ "url": line.strip(), "invalid": False } for line in url_text.get("1.0", "end").splitlines() if line.strip()] # 获取所有非空行
...
style.configure("Custom.Treeview", rowheight=max(24, int(20 * scale)), font=(ui_font_family, 9))
...
def on_tree_select(event: tk.Event) -> None: # 处理树视图选择事件
    ...
    if not selection:
        return
    item = selection[0]
    if treeview.get_children(item): # 如果选中的项有子项，则切换展开/收起状态，否则插入 URL
...
build_tree_items("root", resource_list, open_sub=True) # 构建树视图项，初始展开一级目录
